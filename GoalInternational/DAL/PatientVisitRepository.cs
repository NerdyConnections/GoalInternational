...
namespace GoalInternational.DAL
{
    public class PatientVisitRepository : BaseRepository
    {
        ...
        public List<PatientVisitModel> GetAllPatientVisits(int UserID)
        {
            ...
            //retrieve all Collaborators - Sub-I who want the PI to view his patients.
            //if the physician wants his patients to be viewable by the PI, simply change the Usertype to 'C' in the InviteesMaster table
            List<int> CollUserIDs = new List<int> { 0 };
            ...
            try
            {
                ...
                if (UserType == "PI")
                {
                    ...
                    if (val > 0)
                        ...
                        CollUserIDs = Entities.InviteesMasters.Where(im => im.ParentUserID.HasValue && im.UserID.HasValue && im.ParentUserID == UserID && im.UserType == "C").Select(c => c.UserID.Value).ToList();
                    //PI can see all patients entered by SI and SC
                    PatientVisits = (from pv in Entities.PatientVisits

                                     join pi in Entities.PatientInfoes on pv.PatientID equals pi.id
                                     where pv.UserID == UserID || SCUserIDs.Contains(pv.UserID) || CollUserIDs.Contains(pv.UserID)
                ...
                {
                    /* change request May 19, allow other SC to view other SC's patients*/

                    //retrieve all fellow SCs  - get all users with the same ParentUserID and UserType='SC'

                    SCUserIDs = Entities.InviteesMasters.Where(im => im.ParentUserID.HasValue && im.UserID.HasValue && im.ParentUserID == ParentUserID && im.UserType == "SC").Select(c => c.UserID.Value).ToList();
                    ...
                    PatientVisits = (from pv in Entities.PatientVisits
                                     ...
                                     join pi in Entities.PatientInfoes on pv.PatientID equals pi.id
                                     //display patients entered by himself or PI or SCs
                                     where pv.UserID == UserID || pv.UserID == ParentUserID || SCUserIDs.Contains(pv.UserID)
                                     // display all requests for now where pr.RequestStatus == 2 || pr.RequestStatus == 3  //show only submitted requests (either approved 3 or waiting to be approved  2 in the admin tool)
                                     orderby pv.PatientID ascending
                                     select new PatientVisitModel()
                                     {
                ...
                }//if sub-investigator or Collaborator
                else
                {
                    PatientVisits = (from pv in Entities.PatientVisits
                ...
                foreach (PatientVisitModel item in PatientVisits)
                {
                    ...
                    if (Page1 != null)
                    {
                        ...
                        item.PatientConsentDateStr = Page1.PatientConsentDate;
                    ...
                    //if visit 2 is not ready, not started , evaluate the proper state
                    //no need to do anything though if visit 2 is complete status==3
                    if (item.GIntV2Status == 0 || item.GIntV2Status==1)
                    {
                        ...
                        if (item.GIntV1Status == 3)  //only try to attempt to open if visit 1 is completed , visit 1 cannot be no show so no need to check status =5
                        {
                            ...
                            if (DateTime.TryParse(item.GIntVisit1DateStr, out Completiondate))//if v1 completion date is available
                            {
                                ...
                                //don't care about 8 months due to covid19
                    ...
                    if (item.GIntV3Status == 0 || item.GIntV3Status == 1)
                    {
                        ...
                        if (item.GIntV2Status == 3)  //only try to attempt to open if visit 2 is completed or no show
                        {
                            DateTime Completiondate;
                            DateTime PatientConsentDate;
                            //item.GIntV3Status = 1;//covid 19 measure- open visit 2 immediately , uncomment code below and delete this line if back to normal
                            if (DateTime.TryParse(item.GIntVisit2DateStr, out Completiondate))//if v2 completion date is available
                            {
                                ...
                                if (DateTime.Now.Date >= Completiondate.AddMonths(3).Date)
                                {
                                    ...
                                    if (DateTime.TryParse(item.PatientConsentDateStr, out PatientConsentDate))
                                    {
                                        ...
                                        {
                                            item.GIntV3Status = 1;//set to not started (ready to go) if visit 1 is completed and it completed more than 4 months
                                        }
                                    }
                                }
                                // else if (DateTime.Now.Date > Completiondate.AddMonths(8).Date)
                    ...
                    if (item.GIntV3Status == 2)
                    {
                        ...
                        if (DateTime.TryParse(item.PatientConsentDateStr, out PatientConsentDate))
                        {
                            if (DateTime.Now.Date >= PatientConsentDate.AddMonths(16).Date)
                            {
                                //blocking visit if v3 is not start or not complete in 8 months from patient consent date
                                item.GIntV3Status = 8;
                            }
                    ...
                    //if visit 4 is not started or not ready, evaluate if it should, if it is any other status (not started 1, started 2, completed 3, leave it as is , don't change) 
                    ...
                    if (item.GIntV4Status == 0 || item.GIntV4Status == 1)
                    {
                        if (item.GIntV3Status == 3)  //only try to attempt to open if visit 3 is completed or no show
                        {
                            DateTime Completiondate;
                            DateTime PatientConsentDate;
                            // item.GIntV4Status = 1;//covid 19 measure- open visit 2 immediately , uncomment code below and delete this line if back to normal
                            if (DateTime.TryParse(item.GIntVisit3DateStr, out Completiondate))//if v3 completion date is available
                            {

                                if (DateTime.Now.Date >= Completiondate.AddMonths(3).Date)  //the visit 3 elapsed more than 3 months
                                {
        ...
        {//if visit is lost to follow up.  set visit 3 and 4  to not ready forever.
        ...
        public GIntV1P1Model GetGIntV1P1(int PatientID)
        {
            ...
            if (query != null)
            {
                ...
                gv1p1.LDLCYesNo = query.LDLCYesNo;
        ...
        public GIntV1P4Model GetGIntV1P4(int PatientID)
        {
            ...
            if (query != null)
            {
                ...
                gvmodel.VisitDate = (query2.VisitDate == null) ? null : query2.VisitDate;
                return gvmodel;
        ...
        public GIntV2P1Model GetGIntV2P1(int PatientID)
        {
            ...
            if (query != null)
            {
                ...
                gvmodel.LDLCYesNo = query.LDLCYesNo;
        ...
        public bool SaveV1P1(GIntV1P1Model v1p1model)
        {
            ...
            try
            {
                ...
                if (patient == null)
                {
                    ...
                    pvd.LDLCYesNo = v1p1model.LDLCYesNo;
                ...
                else
                {
                    ...
                    patient.LDLCYesNo = v1p1model.LDLCYesNo;
        ...
        public bool SaveV2P1(GIntV2P1Model model)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    patient.LDLCYesNo = model.LDLCYesNo;
